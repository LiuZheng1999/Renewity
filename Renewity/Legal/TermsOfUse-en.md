# Terms of Use

**Effective date: August 20, 2026**

Welcome to Renewity (the “App”). These Terms of Use (the “Terms”) are an agreement between you and the developer of the App (“we”, “us”, or “our”) governing your download, installation, and use of the App. Please read them carefully. By downloading, installing, accessing, or using the App, you confirm that you have read, understood, and agree to be bound by these Terms.

A web copy matching the in-app text is published on GitHub Pages:  
**https://maoxia-xiang.github.io/Renewity/terms/**

If you do not agree, do not use the App and delete it from your device.

## 1. What the App is

Renewity is a personal tool for recording recurring subscription costs and reminders. It helps you organize streaming, cloud storage, software memberships, and similar charges, review monthly and yearly spend, and receive a local reminder before a charge date. The App requires **iOS 18 or later**.

The App:

- does not provide banking, payment, investment, tax, or financial-advisory services;
- does not pay any third party on your behalf;
- shows amounts, dates, currency conversions, and statistics for personal record-keeping only, which may differ from your actual bills.

## 2. Eligibility and acceptance

You must be at least 13 years old (or the higher minimum age required in your region) to use the App. If you are under the age of majority where you live, you should use the App only with a parent or guardian’s consent and supervision.

When you obtain the App from the Apple App Store, Apple’s Licensed Application End User License Agreement (or the Apple terms then in effect) also applies. If these Terms conflict with Apple’s terms on a matter Apple requires to control, Apple’s terms prevail for that matter.

## 3. License

We grant you a personal, revocable, non-transferable, non-exclusive, limited license to use the App on Apple devices that you own or control, solely in accordance with these Terms.

You may not:

- copy, modify, reverse engineer, decompile, or attempt to extract source code except where the law expressly allows it;
- rent, lend, sell, sublicense, or otherwise provide the App to a third party;
- circumvent or interfere with security or access controls (including App Lock);
- use the App in any unlawful, fraudulent, or infringing way.

## 4. Accounts and sign-in

The App **does not create its own user accounts** and does not require you to register a username or password with us.

If you use iCloud backup, buy Pro through Apple, or unlock the App with Face ID / Touch ID, authentication is handled by Apple on your device or Apple Account. We cannot access your Apple Account password or biometric data.

## 5. Your content

You are responsible for the subscription names, amounts, dates, categories, payment methods, notes, and other information you enter (“User Content”). You represent that you have the right to record that information and that it does not infringe others’ rights or violate the law.

You retain your rights in User Content. You grant us a license to process User Content only as needed to provide recording, statistics, reminders, widgets, and backup on your devices. We do not sell your User Content or use it for advertising.

## 6. Brand names and third-party marks

Third-party names, trademarks, and logos in the App are used only to help you **identify** a service for record-keeping. Their presence:

- does not mean we are affiliated with, sponsored by, or endorsed by the trademark owner;
- does not mean we provide that party’s official app or payment service.

Prices, plans, and billing rules are set by the third party. Reference prices in the App may come from public information or templates and **may be outdated or not apply in your region**. Always rely on your actual bill.

## 7. Pro features and in-app purchases

Some features may require Renewity Pro, such as more than the free subscription limit, custom categories, multiple renewal reminders, changing the display currency / converting totals, and iCloud backup plus file import/export. The features included are those described in the App and on the purchase screen at the time of purchase.

- Purchases, payments, receipts, and auto-renewal are processed by Apple. We do not see your full payment-card details.
- Manage, cancel, or request refunds for auto-renewing subscriptions through your Apple Account / App Store rules.
- Prices are those Apple shows you in your storefront.
- Free features remain available without Pro, subject to quantity or other limits.

Use Restore Purchases with the same Apple Account you used to buy.

## 8. Notifications, widgets, and App Lock

Renewal and trial reminders use **local notifications** provided by the operating system. You can choose whether to remind and how many days in advance, per subscription. The App schedules several upcoming reminders in advance and refreshes that queue when you open the App. The system limits pending notifications, so if you do not open the App for a long time, later reminders may stop. Delivery also depends on notification permission, Low Power Mode, Focus, and similar settings. We do not guarantee that a notification will be delivered.

Home screen widgets read a local snapshot through an App Group so they can show a spending summary.

If you enable App Lock, returning to the App requires Face ID, Touch ID, or your device passcode. This is not a substitute for full-disk encryption or a professional secrecy solution, and it cannot fully protect data if someone already has an unlocked device.

## 9. Third-party services and network requests

Some features may send network requests when you use them, for example:

- reference exchange rates;
- searching companies or App Store apps from keywords you type, to match names and icons;
- loading publicly available icons or artwork.

Those requests may send your search terms, domain names, or technical data such as IP address to the relevant provider. Their practices are governed by their own policies. See the Privacy Policy.

## 10. Disclaimers

The App is provided “as is” and “as available”. To the maximum extent permitted by law, we disclaim all warranties, express or implied, including that:

- the App will be uninterrupted, error-free, fully secure, or compatible with future system versions;
- statistics, currency conversion, and next-billing estimates will match your real bills;
- third-party apps, websites, or icons found via search are appropriate for you or correctly age-rated in your region.

**Nothing in the App is financial, legal, tax, or investment advice.** Whether you keep or cancel a subscription is your decision.

## 11. Limitation of liability

To the maximum extent permitted by law, we are not liable for any indirect, incidental, special, consequential, or punitive damages arising from your use of or inability to use the App, including lost data, interruption, or lost profits.

Our total liability to you will not exceed the amount you actually paid us for the App in the 12 months before the claim (or zero if you only use free features). Some regions do not allow certain limitations; those limitations may not apply to you.

## 12. Termination

You may stop using and delete the App at any time. We may terminate your license if you materially breach these Terms. Provisions that by their nature should survive (including disclaimers, limitations of liability, and governing law) will survive.

Export or back up your data before deleting the App. The App does not live-sync your database to the cloud. Deleting the App, turning off iCloud, or changing devices may make local data unrecoverable unless you have backed up as described in the App.

## 13. Changes

We may update these Terms from time to time. The updated version will be shown in the App and on the GitHub Pages site above, with a new effective date. Continued use after the change means you accept the updated Terms. If you do not agree, stop using the App.

## 14. Governing law

These Terms are interpreted in light of the mandatory consumer-protection laws of the place where you downloaded the App. Where those laws allow, you agree to try to resolve disputes through good-faith discussion first.

## 15. Contact

Questions about these Terms: **philiptrip1975@gmail.com**

Web: https://maoxia-xiang.github.io/Renewity/terms/

Please include “Terms of Use” in the subject line.
