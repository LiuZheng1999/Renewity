...
Renewity (l’« App ») est fournie par le développeur de l’App (« nous »). Cette politique explique comment l’App traite les informations vous concernant.

L’App est conçue pour que **les abonnements restent par défaut sur l’appareil**. Nous n’exploitons pas de serveur de connexion et ne collectons pas de données personnelles via l’App à des fins publicitaires ou de profilage.

Questions : **philiptrip1975@gmail.com**

## 1. Ce que nous ne collectons pas via l’App

Sauf indication contraire, pas de compte chez nous et pas d’envoi vers des serveurs que nous contrôlons de : nom ou e-mail (sauf si vous nous écrivez), données d’abonnements saisies, identifiants publicitaires ou SDK d’analyse pour vous suivre d’une app à l’autre.

L’App **n’inclut pas de SDK publicitaires tiers ni d’outils d’analyse de suivi inter-apps**.
...
Si la devise d’affichage diffère de la devise de référence locale, des cours publics (actuellement `open.er-api.com`) peuvent servir à une estimation, pas à une offre de change.

Lors de « Choisir un service », l’App peut interroger l’API App Store / iTunes, des services de domaine publics (ex. Clearbit) et des CDN d’icônes (icon.horse, unavatar.io, favicone.com, selon les requêtes). **Les mots-clés ou domaines saisis** peuvent y être envoyés. N’y mettez pas de mot de passe ni de numéro de carte.
...
Indiquez « Politique de confidentialité » en objet.
